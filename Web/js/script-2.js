...
$('.chart-branding').data('html',$('.chart-branding').html());$('.action-change-cost-mode').bind('click',function(){if(activeCostMode=='cost_for_nomad'){activeCostMode='cost_for_expat';$('.action-change-cost-mode').text('🤓 Cost for expat');}
else if(activeCostMode=='cost_for_expat'){activeCostMode='cost_for_local';$('.action-change-cost-mode').text('👨‍🌾 Cost for local');}
else if(activeCostMode=='cost_for_local'){activeCostMode='cost_for_family';$('.action-change-cost-mode').text('👶 Cost for family');}
else if(activeCostMode=='cost_for_family'){activeCostMode='cost_for_nomad';$('.action-change-cost-mode').text('👩‍💻 Cost for remote worker');}
$('.filter.cost .choice').data('filter-target',activeCostMode+'_in_usd');setCookie('cost_mode',activeCostMode,86400*365);if(isLoggedIn){$.ajax({dataType:'json',url:'/userApi.php?action=set_cost_mode&cost_mode='+activeCostMode,success:function(reply){}});}
if($('.filter.cost .choice').length>0){showItemsLoading();makeFilters();updateItemsToMatchFilter();}
else{processFilterReplyAndGenerateItems();}});if(typeof preloadedFiltersOnPageState!=='undefined'){if(!empty(preloadedFiltersOnPageState)){loadFiltersPageState(preloadedFiltersOnPageState);updateActiveFiltersBox();updateItemsToMatchFilter();}
...
showLoading();showItemsLoading();updateItemsToMatchFilter();$(this).remove();});$('html').bind('keyup',function(e){if(!mapIs3d)return;if(e.keyCode==17){$('.notice-hold-control-to-drag-3d-map').hide();}});$('body').on('click','.grid .item .action-hide-item',function(){$(this).parent().fadeOut(function(){$('.item.not-a-city').each(function(){html=$(this)[0].outerHTML;$(this).next('.item').after(html);$(this).remove();});});});$('body').on('click','.tab-reviews .action-add-review',function(){if(empty($('.tab-reviews .review-editor').val()))return;data={};data['action']='change';data['slug']=activeItemSlugLong;data['key']='review';data['value']=$('.tab-reviews .review-editor').val();data['value']=str_replace(activeItemName.toLowerCase(),activeItemName,data['value']);$(this).text('Edit review');if(!empty(userUserId)){md5UserId=md5(userUserId+'1986');$('.tab-reviews .review.'+md5UserId).remove();}
else{md5UserId='';}
...
if($('.tab-reviews .review-editor').val().length>100){activeAjax=$.ajax({async:true,url:'/api/v2/change/',type:'POST',dataType:'json',data:data}).done(function(reply){if(typeof reply.spam_detected!=='undefined'){setCookie('spammer','true',86400*365);}});}
...
$('html').removeClass('grid-view').removeClass('map-view').removeClass('map-3d-view').removeClass('list-view').addClass(activeView+'-view');if(!mapIsLoaded)loadMap($(this).data('longitude'),$(this).data('latitude'),4,false);activeView='map';startLatitude=$(this).data('latitude');startLongitude=$(this).data('longitude');$('.action-clear-all-filters').trigger('click');closeItemModal();});$('body').on('mouseenter touchstart','.item',function(){var slug=$(this).data('slug');if(itemType!='city')return;if(empty(slug))return;if(typeof modalCityCache[slug]!=='undefined')return;url='/modal/city/'+slug+'?'+date('Y-m-d')+'_11_'+Math.round(date('H')/6);preloadAjax=$.ajax({url:url,success:function(html){if(typeof html!=='undefined'){if(!empty(html)){modalCityCache[slug]={};modalCityCache[slug]['epoch']=time();modalCityCache[slug]['html']=html.slice(0);}}}},slug);});document.addEventListener('gesturestart',function(e){e.preventDefault();});if(windowSize().width<700){itemsToAddPerInfinityLoad=16;}
if(itemType=='city'||itemType=='region'||itemType=='country'){if(defaultSortOrder!=activeSortOrder){$('.action-select-sort-order div').removeClass('active');$('.action-select-sort-order div.'+activeSortOrder).addClass('active');}
...
$('.filters .filter-origin-container input').val(userCityShort);$('.sort-by select option[data-sort="'+'flight_price_usd'+'"]').text('✈️ flight price ('+userCityShort+')');$('.filter.flight_price_usd .choice').each(function(){$(this).data('tooltip',str_replace('{userCity}',userCityShort,$(this).data('original-tooltip')));});setSelfMapMarker();}
...
action='👀 Opened '+activeView+' view';$.ajax({url:"/userApi.php",type:'POST',dataType:'json',data:{action:'log_user_activity',log_action:action,hash:md5(action+'xoyo')},context:document.body}).done(function(reply){});});$('.action-download-csv').bind('touchend mouseup',function(e){e.stopPropagation();e.preventDefault();$(this).blur();downloadCSV();});$('.action-toggle-filters').bind('touchend mouseup',function(e){$('.toolreview-ui').hide();e.stopPropagation();e.preventDefault();$(this).blur();action='🔍 Opened filters';$.ajax({url:"/userApi.php",type:'POST',dataType:'json',data:{action:'log_user_activity',log_action:action,hash:md5(action+'xoyo')},context:document.body}).done(function(reply){});if(isLoggedIn)$('html').removeClass('show-grid-side');$('html').removeClass('has-head-image').removeClass('pageType-country').removeClass('pageType-list').removeClass('pageType-region').addClass('pageType-cities');consoleLog("6	$('.head-image').hide();");$('.head-image').hide();$('.new-user-hero').hide();$('.new-user-hero-press-featured').hide();if($('html').hasClass('new-user')){$('html').removeClass('new-user');setCookie('new-user','no',86400*365);if(windowSize().height>1000){if(!empty(itemsData)){$('.view-'+activeView+'.infinity-scroll').show();processFilterReplyAndGenerateItems(0,true);$('.view-'+activeView+'.infinity-scroll').hide();}
...
lazyload();});$('body').on('click','.action-cta-for-non-user',function(e){if(!$('html').hasClass('logged-in')){e.preventDefault();e.stopPropagation();if(!empty($(this).data('cta-url'))){window.open($(this).data('cta-url'),'_new');}
else{window.open('https://join.nomadlist.com/','_new');}}
...
$('.filter.flight_price_usd .choice').each(function(){$(this).data('tooltip',str_replace('{userCity}',userCountry,$(this).data('original-tooltip')));});setSelfMapMarker();setTimeout(function(){getSkyPickerFlights();clearAllMapMarkers();showItemsLoading();showLoading();},1000);});var timeoutUserWaitForupdateItemsToMatchFilter=setTimeout(function(){},0);choiceDragMouseDown=false;choiceDragActive=true;$('body').on('click','.filters .choice',function(e){if(typeof pageRef==='undefined')pageRef='';if(!$(this).hasClass('active')&&getCookie('visit-count')>=showSignUpModalAfterVisitCount&&$('.filters .choice.active').length>=1&&!$('html').hasClass('logged-in')&&!(pageRef.indexOf('google')>-1)&&!(pageRef.indexOf('bing')>-1)&&!(pageRef.indexOf('duck')>-1)&&!(pageRef.indexOf('yandex')>-1)){action='💳 Opened sign up modal when using filters '+'('+signUpAbTestVariation+')';$.ajax({url:"/userApi.php",type:'POST',dataType:'json',data:{action:'log_user_activity',log_action:action,hash:md5(action+'xoyo')},context:document.body}).done(function(reply){});openSignUpModal('🔑 <strong>Unlock the <div style="margin-left:7px;margin-right:7px;display:inline-block;font-size:0.9em;font-weight:bold;border-radius:6px; background:var(--input-background-color);/*border:1px solid var(--input-border-color);*/;box-shadow:0 1px 2px 0 rgba(0, 0, 0, 0.1) !important;padding:7px;padding-left:14px;padding-right:14px;">'+$(this).text()+'</div></strong> and 200+ other filters','basic');$('.sign-up-notice-free-limit-reached').show();$('.sign-up-cta-bottom').show();$('.action-close-modal-pay').bind('click',function(){setTimeout(function(){$('.sign-up-cta-bottom').show();},15000);})
...
consoleLog("2	$('.head-image').hide();");$('.head-image').hide();$('.new-user-hero').hide();$('.new-user-hero-press-featured').hide();$('html').removeClass('has-head-image');$('.notice').remove();latestFilterTargetSelected=$(this).data('filter-target');activeSearch='';$('input.search').val('');if(!$('html').hasClass('logged-in')){if($(this).hasClass('members-only')||$(this).parent().hasClass('members-only')||$(this).parent().parent().hasClass('members-only')||$(this).parent().parent().parent().hasClass('members-only')||$(this).parent().parent().parent().parent().hasClass('members-only')||$(this).parent().parent().parent().parent().parent().hasClass('members-only')){action='💳 Opened sign up modal when clicking members-only filters '+'('+signUpAbTestVariation+')';$.ajax({url:"/userApi.php",type:'POST',dataType:'json',data:{action:'log_user_activity',log_action:action,hash:md5(action+'xoyo')},context:document.body}).done(function(reply){});$('.sign-up-notice-free-limit-reached').show();openSignUpModal('🔑 <strong>Unlock the <div style="margin-left:7px;margin-right:7px;display:inline-block;font-size:0.9em;font-weight:bold;border-radius:6px;border:1px solid var(--input-border-color);padding:7px;padding-left:14px;padding-right:14px;">'+$(this).text()+'</div></strong> and 200+ other filters','basic');consoleLog('This filter is limited to members only, shown due to visit count > '+getCookie('visit-count'));return false;}}
...
latestFilterTargetSelected='visa_free_for';showItemsLoading();updateItemsToMatchFilter();});setTimeout(function(){lazyload();},100);trackEvent('pageview',pageUrl);trackEvent('visit',pageUrl);trackEvent('action',pageUrl);if(typeof pageHash==='undefined')pageHash='';if(!empty(window.location.hash)||!empty(pageHash)){if(!empty(pageHash)){var hash=pageHash;}
...
else{searchFiltersTimeout=setTimeout(function(that){var query=$(that).val().toLowerCase();if(windowSize().width<800){var maxResults=10;}
else{var maxResults=30;}
var i=0;$('.search-filters-results').hide();$('.search-filters-results').html('');labelDupes=[];var matches=[];$('.filters .choice:not(.active)').each(function(){if(i>maxResults)return true;if($(this).parent().hasClass('month'))return true;if(empty(query)){if(Math.round(Math.random()*10)!=1){return true;}}
...
match['labelQuery']=labelQuery;match['label']=label;match['tooltip']=tooltip;matches.push(match);i++;}});consoleLog('matches',matches);if(query.indexOf('timez')>-1){consoleLog('timezone search');}
...
rowHTML=rowHTML+'<div data-label="'+match['labelQuery']+'" data-tooltip="'+match['tooltip']+'" class="search-filters-result tooltip-set">'+match['label']+'</div>';rowInColumnCounter++;if((i+1)==maxResults)break;}
...
if(zoom>14)labelHtml='<div '+dataUrl+' class="marker marker-label-solid" style="left:'+Math.round(x)+'px;top:'+(Math.round(y)+10)+'px;">'+place['name']+'</div>';emoji='📍';$('.map-markers').append('<div class="marker marker-place" style="left:'+Math.round(x)+'px;top:'+Math.round(y)+'px;">'+emoji+'</div>'
+
labelHtml);}}
...
zoomMultiplier=zoom;if(zoomMultiplier<1)zoomMultiplier=1;for(b=0;b<avatarsUserIds.length;b++){avatar=avatarsUserIds[b];if(zoom<5||mapIs3d){if(mapIs3d){avatarImgString=avatarImgString+'<div class="avatar" style="z-index:'+zIndex+';position:absolute;left:0px;top:'+(-4*b^(zoomMultiplier))+'px;background-image:url(\'/assets/img/users/'+avatar+'-100px.jpg\')"></div>';}
else{avatarImgString=avatarImgString+'<div class="avatar" style="z-index:'+zIndex+';position:absolute;left:'+(-0.5*b)+'px;top:'+(-4*b)+'px;background-image:url(\'/assets/img/users/'+avatar+'-100px.jpg\')"></div>';}}
else{avatarImgString=avatarImgString+'<div class="avatar" style="background-image:url(\'/assets/img/users/'+avatar+'-100px.jpg\')"></div>';}}
custom=avatarImgString;labelHtml='';labelHtml='<div data-slug="'+short_slug+'" data-slug-long="'+long_slug+'" class="map-marker map-marker-label '+short_slug+'  score-'+normalizedScore+'"><img src="https://nomadlist.com/assets/img/places/'+long_slug+'-100px.jpg" /> '+name+'</div>';extraStyles='';if(custom.length>3)extraStyles='border-radius:21px;';markerHtml='<div  data-slug="'+short_slug+'" data-slug-long="'+long_slug+'" class="map-marker map-marker-avatar '+short_slug+' " style="z-index:'+(zIndex+100)+';'+extraStyles+'">'+
custom
+'</div>';}
else{labelHtml='';labelHtml='<div data-slug="'+short_slug+'" class="map-marker map-marker-label '+short_slug+'  score-'+normalizedScore+'" ><img src="https://nomadlist.com/assets/img/places/'+long_slug+'-100px.jpg" /> '+name+'</div>';extraStyles='';if(custom.length>3)extraStyles='border-radius:21px;';markerHtml='<div data-slug="'+short_slug+'" data-slug-long="'+long_slug+'" class="map-marker map-marker-data score-'+normalizedScore+' '+short_slug+' " style="'+extraStyles+'">'+custom+'</div>'+labelHtml;}
...
tooltipText='';if($(this).data('tooltip')){tooltipText='class="toolreview-set" data-tooltip="'+$(this).data('tooltip')+'"';}
else if($(this).parent().data('tooltip')){tooltipText='class="toolreview-set" data-tooltip="'+$(this).parent().data('tooltip')+'"';}
html=html+'<div class="active-filter tooltip-set" '+tooltipText+' data-text="'+$(this).text()+'" data-filter-target="'+$(this).data('filter-target')+'">'+thisHtml+'<img class="action-remove-this-filter" src="/assets/times-circle.svg">'+'</div>';consoleLog('<div class="active-filter tooltip-set" '+tooltipText+' data-text="'+$(this).text()+'" data-filter-target="'+$(this).data('filter-target')+'">'+thisHtml+'<img class="action-remove-this-filter" src="/assets/times-circle.svg">'+'</div>');});$('.filters .filter select.choices.active').each(function(){if(!empty($(this).find(':selected').data('filter-target'))){thisHtml='🛂 Visa free for citizens of '+$(this).find(':selected').html();html=html+'<div class="active-filter" data-text="'+$(this).find(':selected').text()+'" data-filter-target="'+$(this).find(':selected').data('filter-target')+'">'+thisHtml+'<span class="action-remove-this-filter" src="/assets/times-circle.svg">'+'</div>';}});count='';if(itemType=='city'){if(typeof itemsData!=='undefined'){if(typeof itemsData.cities!=='undefined'){count=itemsData.cities.length;}}}
...
if(count==0)count='';if(new Date().getHours()<7||new Date().getHours()>20){emoji='🌃';}
else if(new Date().getHours()<11||new Date().getHours()>17){emoji='🌇';}
else{emoji='🌆';}
if(html!=''){html=html+'<div class="action-clear-all-filters">Clear '+number_format(count)+' results</div>';}
else{}
$('.active-filters-box').html(html);if(typeof latestFilterTargetSelected!=='undefined'){if($('.active-filters-box div.active-filter[data-filter-target="'+latestFilterTargetSelected+'"]').length>0){$('.active-filters-box').animate({scrollLeft:($('.active-filters-box div.active-filter[data-filter-target="'+latestFilterTargetSelected+'"]').offset().left-$('.active-filters-box').offset().left)},0);}}}
function updateItemsToMatchFilter(){consoleLog('updateItemsToMatchFilter');if(hasEmojis(activeSearch))return;var latitude;var longitude;$('.view-container-heading').hide();$('.notice-no-items-found').hide();monthIsSelected=false;if($('.filters .filter.month .choice.active').length>0)monthIsSelected=true;monthSelected=$('.filter.month .choice.active').data('filter-value');data={};data['view']=activeView;data['filters']=makeFilters();if(activeView!='grid'){showLoading();}
...
if(data['sort']=='users_now'){data['sort']='users_now_count';data['order']='desc';}
...
activeAjax=$.ajax({url:'/api/v2/filter/all?php8',type:'POST',dataType:'json',data:{data:messUpString((JSON.stringify(data).split("").join(""))+'Gt8SkZ31e6'+(md5(JSON.stringify(data)+''+(Math.round(Math.floor(new Date().getTime()/1000)/100000))+'nomadlist')))}}).done(function(reply1){consoleLog('runtime',reply1.runtime.total);$('.runtime').text(Math.round(reply1.runtime.total)+'ms');$('.runtime').show();if(!reply1.success){alert(reply1.message);return;}
if(reply1.activeAjaxID!=activeAjaxID){return;}
activeAjaxID=0;updateItemsToMatchFilterAJAXCallback(reply1);});}
function updateItemsToMatchFilterAJAXCallback(updateItemsToMatchFilterAJAXCallbackReply){itemsData=updateItemsToMatchFilterAJAXCallbackReply;if(itemsData.cities.length){if(new Date().getHours()<7||new Date().getHours()>20){emoji='🌃';}
else if(new Date().getHours()<11||new Date().getHours()>17){emoji='🌇';}
else{emoji='🌆';}
...
if(generateItemsInJS&&activeView=='grid'){$(window).scroll(function(){if(modalOpen)return;if(!empty(infinityTimeout)){consoleLog('infinity scroll: clearTimeout(infinityTimeout)');clearTimeout(infinityTimeout);}
if($(window).scrollTop()>window.innerHeight){$('.loading_spinner_2018.infinity-scroll').addClass('show');}
consoleLog("INFINITY SCROLL 1");infinityTimeout=setTimeout(function(){consoleLog('infinity scroll: infinityTimeout');if(activeView=='map')return;if(($(window).scrollTop()+window.innerHeight*3)>=($(document).height())&&$(window).scrollTop()>10){if(typeof itemsData==='undefined'){onFirstPageLoadGetItemsData();return;}
if(typeof itemsData.cities==='undefined'){onFirstPageLoadGetItemsData();return;}
if(typeof $('.view.'+activeView+' .item:not(.template):not(.item-aux):not(.not-a-city)')==='undefined')return;if(itemType=='country'){dataToUse=itemsData.countries;}
else if(itemType=='region'){dataToUse=itemsData.regions;}
else{dataToUse=itemsData.cities;}
if(dataToUse.length==$('.view.'+activeView+' .item:not(.template):not(.item-aux):not(.not-a-city)').length){$('.loading_spinner_2018.infinity-scroll').removeClass('show').addClass('loaded-all-items-already');consoleLog('infinity scroll: already showed ALL items from the return data! don\'t infinity scroll');return;}
setTimeout(function(){if(activeView=='grid'&&(itemType=='city'||itemType=='region'||itemType=='country')){if(typeof($('.view.grid .item:not(.template):not(.item-aux):not(.not-a-city)')!=='undefined')){consoleLog('infinity scroll: hit processFilterReplyAndGenerateItems',$('.view.'+activeView+' .item:not(.template):not(.item-aux):not(.not-a-city)').length);processFilterReplyAndGenerateItems(($('.view.'+activeView+' .item:not(.template):not(.item-aux):not(.not-a-city)').length));}}
else{$('.loading_spinner_2018.infinity-scroll').removeClass('show').addClass('loaded-all-items-already');}},250);}},250);});}
function getSkyPickerFlightsForRoute(){return;if($('.tab-flights table').length>0)return;$('.tab-flights').text(' ');$('.loading_spinner-item').addClass('show');$('.loading_spinner-item').show();$('.loading_spinner-item .loading_message').html("Loading flights for "+userCity+" to "+activeItemName+".<br/>Here's a cute puppy 🐶 to look at");to=activeItemName;skyPickerAjax=$.ajax({async:true,'url':'https://api.skypicker.com/flights',type:'GET',dataType:'json',data:{'v':2,'sort':'date','asc':1,'locale':'us','currency':'USD','curr':'USD','daysInDestinationFrom':'','daysInDestinationTo':'','partner':'nomadlist','children':0,'infants':0,'flyFrom':(Math.round(userLatitude*100)/100)+'-'+(Math.round(userLongitude*100)/100)+'-120km','to':to,'featureName':'map','typeFlight':'oneway','returnFrom':'','returnTo':'','adults':1,}}).done(function(skyPickerReply){$('.loading_spinner-item').addClass('show');if(empty(skyPickerReply.data)){$('.modal.item .tab-flights').html("<div class=\"no-flights-found\"><i class=\"fa fa-plane\"></i><p>Sorry, no flights found from "+userCity+' to '+activeItemName+'</p><p>Try changing your departure city (at the top of the filters)</p></div>');$('.loading_spinner-item').removeClass('show');return;}
$('.modal.item .tab-flights').html('<table></table>');for(var i=0;i<skyPickerReply.data.length;i+=1){thisFlight=skyPickerReply.data[i];if(((thisFlight.duration.total-(Math.floor(thisFlight.duration.total/60/60)*60*60))/60)!==0){flight_time=Math.floor(thisFlight.duration.total/60/60)+'h'+
((thisFlight.duration.total-(Math.floor(thisFlight.duration.total/60/60)*60*60))/60)+'m';}
else{flight_time=Math.floor(thisFlight.duration.total/60/60)+'h';}
airlineLogos='';for(var x=0;x<thisFlight.airlines.length;x+=1){airlineLogos=airlineLogos+'<img src="https://images.kiwi.com/airlines/64/'+thisFlight.airlines[x]+'.png" />';}
via='';for(var x=0;x<thisFlight.route.length;x+=1){if(empty(thisFlight.route[x]))return;if(thisFlight.route[x].cityTo!==thisFlight.cityTo){via=via+'via '+thisFlight.route[x].cityTo+' ('+thisFlight.route[x].flyTo+') <br/>'}}
if(!empty(via)){via='<br/>'+via+'';}
url=thisFlight.deep_link;$('.modal.item .tab-flights table').append('<tr data-url="'+url+'">'+
'<td class="price">'+
'$'+thisFlight.price
+'</td>'+
'<td class="logo">'+
...
+'</td>'+
'<td class="times">'+
'🗓 '+'<em>'+date('D j M',thisFlight.dTime)+'</em>'+
' (in '+(Math.floor((thisFlight.dTime-time())/60/60/24))+'d)'+
'<br/>'+
date('H:i',thisFlight.dTime)
+' &mdash; '+
date('H:i',thisFlight.aTime)
+'</td>'+
'<td class="from">'+
'🛫 '+'<em>'+thisFlight.cityFrom+' ('+thisFlight.flyFrom+')'+'</em>'+
via
+'</td>'+
'<td class="to">'+
'🛬 '+thisFlight.cityTo+' ('+thisFlight.flyTo+')'
+'</td>'+
'<td class="duration">'+
'⌚️ '+flight_time
+'</td>'+
'</tr>');}
if((i+1)>skyPickerReply.data.length){$('.loading_spinner-item').removeClass('show');$('.loading_spinner .loading_message').text('');}});}
function getSkyPickerFlights(doItInBackground){if(!doItInBackground){$('.loading_spinner-general').addClass('show');$('.loading_spinner-general .loading_message').show();$('.loading_spinner-general .loading_message').html("Loading flights from "+userCity+".<br/>Here's a cute cat 🐈 to look at");}
skyPickerAjax=$.ajax({async:true,'url':'https://api.skypicker.com/aggregation_flights',type:'GET',dataType:'json',data:{'adults':1,'affilid':'nomadlist','asc':1,'cabinClass':'economy','children':0,'dateFrom':date('d/m/Y',strtotime("+30 days")),'dateTo':date('d/m/Y',strtotime("+44 days")),'daysInDestinationFrom':'','daysInDestinationTo':'','featureName':'map','flyFrom':(Math.round(userLatitude*100)/100)+'-'+(Math.round(userLongitude*100)/100)+'-120km','infants':0,'locale':'us','one_per_date':0,'oneforcity':1,'partner':'nomadlist','returnFrom':date('d/m/Y',strtotime("+30 days")),'returnTo':date('d/m/Y',strtotime("+44 days")),'simplified_call':1,'sort':'price','to':'anywhere','typeFlight':'return','v':3,'wait_for_refresh':0,}}).done(function(skyPickerReply){skyPickerFlightPricesClientSideCache=skyPickerReply;skyPickerFlightPricesClientSideCache['epoch']=time();localStorage.setItem('skyPickerFlightPricesClientSideCache',JSON.stringify(skyPickerFlightPricesClientSideCache));if(!doItInBackground){$('.loading_spinner .loading_message').text('');processSkyPickerFlightPricesReply();}});}
function processSkyPickerFlightPricesReply(){consoleLog('processSkyPickerFlightPricesReply');showLoading();if(empty(itemsData)){return;}
if(empty(skyPickerFlightPricesClientSideCache)){getSkyPickerFlights();return;}
showItemsLoading();showLoading();processedItemsData=clone(itemsData);newItemsData=clone(itemsData);newItemsData.cities=[];for(i=0;i<processedItemsData.cities.length;i++){item=processedItemsData.cities[i];skyPickerCityId='';if(item['country_code']=='UK'){for(var key in skyPickerFlightPricesClientSideCache.data){if(key.substr(key.length-3)=='_gb'){if(makeUrlSlug(item['name'])==key.substr(0,item['name'].length)){skyPickerCityId=key;}}}}
else if(item['country_code']=='CA'){for(var key in skyPickerFlightPricesClientSideCache.data){if(key.substr(key.length-3)=='_ca'){if(makeUrlSlug(item['name'])==key.substr(0,item['name'].length)){skyPickerCityId=key;}}}}
else if(item['country_code']=='AU'){for(var key in skyPickerFlightPricesClientSideCache.data){if(key.substr(key.length-3)=='_au'){if(makeUrlSlug(item['name'])==key.substr(0,item['name'].length)){skyPickerCityId=key;}}}}
else if(item['country_code']=='BR'){for(var key in skyPickerFlightPricesClientSideCache.data){if(key.substr(key.length-3)=='_br'){if(makeUrlSlug(item['name'])==key.substr(0,item['name'].length)){skyPickerCityId=key;}}}}
else if(item['country_code']=='AR'){for(var key in skyPickerFlightPricesClientSideCache.data){if(key.substr(key.length-3)=='_ar'){if(makeUrlSlug(item['name'])==key.substr(0,item['name'].length)){skyPickerCityId=key;}}}}
else if(item['country_code']=='US'){skyPickerCityId=makeUrlSlug(item['name'])+'_'+makeUrlSlug(item['state_code'])+'_'+makeUrlSlug(item['country_code']);}
else{skyPickerCityId=makeUrlSlug(item['name'])+'_'+makeUrlSlug(item['country_code']);}
if(item['slug'].indexOf('bali')>-1)skyPickerCityId='denpasar_id';if(item['slug'].indexOf('penang')>-1)skyPickerCityId='penang_my';item['flight_price_usd']=null;item['flight_price_airport']=null;item['flight_price_usd_normalized']=null;if(!empty(skyPickerFlightPricesClientSideCache.data[skyPickerCityId])){item['flight_price_usd']=skyPickerFlightPricesClientSideCache.data[skyPickerCityId];newItemsData.cities.push(item);}}
maxFlightPrice=99999999;if($('.filters .filter.flight_price_usd .choice.active').data('filter-type')=='less-than'){maxFlightPrice=$('.filters .filter.flight_price_usd .choice.active').data('filter-max');}
processedItemsData=clone(newItemsData)
newItemsData=clone(newItemsData)
newItemsData.cities=[];for(i=0;i<processedItemsData.cities.length;i++){item=processedItemsData.cities[i];if(item['flight_price_usd']>maxFlightPrice)continue;newItemsData.cities.push(item);}
processedItemsData=clone(newItemsData);newItemsData=clone(newItemsData);newItemsData.cities=[];allFlightPrices=[];for(i=0;i<processedItemsData.cities.length;i++){item=processedItemsData.cities[i];allFlightPrices.push(item['flight_price_usd']);}
outliersRemovedFlightPrices=removeOutliers(allFlightPrices);lowestFlightPrice=9999999;highestFlightPrice=0;for(i=0;i<outliersRemovedFlightPrices.length;i++){price=outliersRemovedFlightPrices[i];if(price<lowestFlightPrice)lowestFlightPrice=price;if(price>highestFlightPrice)highestFlightPrice=price;}
newItemsData=clone(newItemsData)
newItemsData.cities=[];for(i=0;i<processedItemsData.cities.length;i++){item=processedItemsData.cities[i];item['flight_price_usd_normalized']=(item['flight_price_usd']-lowestFlightPrice)/(highestFlightPrice-lowestFlightPrice)
newItemsData.cities.push(item);}
if(activeSort=='flight_price_usd'){newItemsData['return']=sortBySubkey(newItemsData['return'],'flight_price_usd');if(activeSortOrder=='desc'){newItemsData['return']=newItemsData['return'].reverse();}}
itemsData=clone(newItemsData);if(itemsData.cities.length==0){showNoItemsFound();}
mapMarkers=[];consoleLog('A processFilterReplyAndGenerateItems');processFilterReplyAndGenerateItems(0,true);}
...
else if(empty(itemsData.slugs)){document.title='Nomad List';history.replaceState(null,null,rootUri);$('.no-items-found').show();}
else if(itemsData.slugs.length==0){document.title='Nomad List';history.replaceState(null,null,rootUri);$('.no-items-found').show();}
...
if(in_array(activeSortAttributeToShow,defaultAttributesDisplayedInItemBox)){$('.view.list thead .custom').data('sort',defaultSort);$('.view.list thead tr .custom').text('🎒 Nomad<span class="hide-on-mobile"> Score</span>');}
...
else{$('.filter-actions .action-toggle-filters-show-cities').text('Show '+number_format(itemsData.cities.length)+' cities');}}
...
itemTemplated=itemTemplated.replace('{rotate-deg}',(Math.floor(Math.random()*Math.floor(6))-3)/4);if(itemType=='country'||itemType=='region'){if(activeSort!=defaultSort){itemTemplated=itemTemplated.replace('{data-url}','/'+item['short_slug']+'?sort='+activeSort+'&order='+activeSortOrder);}
...
bgSrcset='https://nomadlist.com/assets/img/places/'+item['long_slug']+'-300px.jpg 300w, https://nomadlist.com/assets/img/places/'+item['long_slug']+'-600px.jpg 600w';view='grid';itemTemplated=itemTemplated.replace('{bg-data-srcset}',bgSrcset);if(!empty(item[activeCostMode+'_in_usd'])){var converted=item[activeCostMode+'_in_usd']*exchangeRatesUSD[userCurrency.toUpperCase()];if(exchangeRatesUSD[userCurrency.toUpperCase()]>=1000){converted=Math.round(converted/1000)*1000;if(converted>=1000000){converted=number_format(converted/1000000,2)+'M';}
if(converted>=1000){converted=number_format(converted/1000)+'k';}}
else{converted=number_format(converted);}
if(userLanguage=='ZH'){itemTemplated=itemTemplated.replace('{bottom-right}','<span class="price" data-usd="'+item[activeCostMode+'_in_usd']+'">'+exchangeRatesSymbols[userCurrency.toUpperCase()]+''+converted+'</span> / 月');}
else{itemTemplated=itemTemplated.replace('{bottom-right}','<span class="price" data-usd="'+item[activeCostMode+'_in_usd']+'">'+exchangeRatesSymbols[userCurrency.toUpperCase()]+''+converted+'</span> / mo');}}
else{itemTemplated=itemTemplated.replace('{bottom-right}','');}
airQualityExclamationMark='';aqiRedIfBad='';if(monthIsSelected){if(item['air_quality_month_'+monthSelected]>75){aqiRedIfBad='red';airQualityExclamationMark='<span class="smog-emoji">😷</span>';}
aqi=Math.round(item['air_quality_month_'+monthSelected]);}
if(!monthIsSelected){if(item['air_quality_now']>75){aqiRedIfBad='red';airQualityExclamationMark='<span class="smog-emoji">😷</span>';}
...
sweatEmoji='';if(!monthIsSelected){if((item['temperatureC']>=26&&item['humidity']>=80)||item['temperatureC']>=30){sweatEmoji='<span class="sweat-emoji">🥵</span>';}}
else{if((item['climate_temperatureC_month_'+monthSelected]>=26||item['climate_humidity_month_'+monthSelected]>=80)||item['climate_temperatureC_month_'+monthSelected]>=30){sweatEmoji='<span class="sweat-emoji">🥵</span>';}}
...
+'</span>'+'<span class="unit metric">'+number_format(item['climate_temperatureC_month_'+monthSelected])+'°</span><span class="unit imperial">'+number_format(item['climate_temperatureF_month_'+monthSelected])+'°F</span></span>'+sweatEmoji+aqiHtml+monthText);}
...
+'</span>'+'<span class="unit metric">'+Math.round(number_format(item['temperatureC']))+'°</span><span class="unit imperial">'+Math.round(number_format(item['temperatureF']))+'°</span></span>'+sweatEmoji+aqiHtml);}
...
itemTemplated=itemTemplated.replace('{long-slug}',item['slug']);itemTemplated=itemTemplated.replace('{slug}',item['short_slug']);if(typeof userLanguage==='undefined'){userLanguage='EN';}
if(userLanguage=='ZH'){if(!empty(item['state_chinese'])){itemTemplated=itemTemplated.replace('{itemName}','<a href="/'+item['short_slug']+'" title="'+item['name']+' for Digital Nomads">'+item['name_chinese']+'<span class="state long">'+item['state_chinese']+'</span></a>');}
else{itemTemplated=itemTemplated.replace('{itemName}','<a href="/'+item['short_slug']+'" title="'+item['name']+' for Digital Nomads">'+item['name_chinese']+'</a>');}}
else if(!empty(item['state_code'])&&item['state_code'].length<3){itemTemplated=itemTemplated.replace('{itemName}','<a href="/'+item['short_slug']+'" title="'+item['name']+' for Digital Nomads">'+item['name']+', <span class="state">'+item['state_code']+'</span></a>');}
else if(!empty(item['state_code'])){itemTemplated=itemTemplated.replace('{itemName}','<a href="/'+item['short_slug']+'" title="'+item['name']+' for Digital Nomads">'+item['name']+', <span class="state long">'+item['state_code']+'</span></a>');}
else{itemTemplated=itemTemplated.replace('{itemName}','<a itemprop="url" href="/'+item['short_slug']+'" title="'+item['name']+' for Digital Nomads">'+item['name']+'</a>');}
if(itemType!='country'&&itemType!='region'){if(userLanguage=='ZH'){itemTemplated=itemTemplated.replace('{itemSubAlt}',item['country_chinese']);itemTemplated=itemTemplated.replace('{itemSub}',item['country_chinese']);}
else{itemTemplated=itemTemplated.replace('{itemSubAlt}',item['country']);itemTemplated=itemTemplated.replace('{itemSub}',item['country']);}}
...
else{itemTemplated=itemTemplated.replace('{label-main-score}','⭐️ Overall<span class="hide-on-mobile"> Score</span>');itemTemplated=itemTemplated.replace('{rating-main-score-width}',(item['overall_score']/5*100)+'%');itemTemplated=itemTemplated.replace('{rating-main-score-r}',Math.round(item['overall_score']));}
itemTemplated=itemTemplated.replace('{rating-internet-score-r}',(Math.round(item['internet_score'])));itemTemplated=itemTemplated.replace('{rating-internet-score-width}',(item['internet_score']/5*100)+'%');itemTemplated=itemTemplated.replace('{rating-cost-score-r}',(Math.round(item['cost_score'])));itemTemplated=itemTemplated.replace('{rating-cost-score-width}',(item['cost_score']/5*100)+'%');itemTemplated=itemTemplated.replace('{rating-fun-score-r}',(Math.round(item['leisure_quality'])));itemTemplated=itemTemplated.replace('{rating-fun-score-width}',(item['leisure_quality']/5*100)+'%');itemTemplated=itemTemplated.replace('{rating-safety-score-r}',(Math.round(item['safety_level'])));itemTemplated=itemTemplated.replace('{rating-safety-score-width}',(item['safety_level']/5*100)+'%');if(itemType=='country'&&item['users_now_count']>=30){itemTemplated=itemTemplated.replace('{top-left}','<div class="popular-city-badge"></div>');}
else if(itemType=='city'&&item['users_now_count']>=20){itemTemplated=itemTemplated.replace('{top-left}','<div class="popular-city-badge"></div>');}
else if(itemType=='city'&&item['last_2_year_growth']>=10){itemTemplated=itemTemplated.replace('{top-left}','<div class="growing-city-badge"></div>');}
else{itemTemplated=itemTemplated.replace('{top-left}','<div class="rank">'+item['rank']+'</div>');}
viewGridHtmlBuffer=viewGridHtmlBuffer+itemTemplated;itemsGeneratedCount++;}}
...
if(!empty(item[activeSortAttributeToShow])){if(activeSortAttributeToShow=='climate_koppen_name'){if(item[activeSortAttributeToShow]=='Tropical')score=0;if(item[activeSortAttributeToShow]=='Dry')score=2;if(item[activeSortAttributeToShow]=='Temperature')score=3;if(item[activeSortAttributeToShow]=='Temperate')score=3;if(item[activeSortAttributeToShow]=='Continental')score=4;if(item[activeSortAttributeToShow]=='Polar')score=5;addMarkerOnMapForItem(item['long_slug'],item['short_slug'],item['name'],'',item['latitude'],item['longitude'],score,displayText,activeSortAttributeToShow);}
else if(activeSortOrder=='asc'){if(activeSortDataType=='score'){addMarkerOnMapForItem(item['long_slug'],item['short_slug'],item['name'],item['state_code'],'',item['latitude'],item['longitude'],5-item[activeSortAttributeToShow+'_normalized']*5,displayText,activeSortAttributeToShow);}
else if(activeSortDataType=='value'){addMarkerOnMapForItem(item['long_slug'],item['short_slug'],item['name'],item['state_code'],'',item['latitude'],item['longitude'],5-item[activeSortAttributeToShow+'_normalized']*5,displayText,activeSortAttributeToShow);}
else{addMarkerOnMapForItem(item['long_slug'],item['short_slug'],item['name'],item['state_code'],'',item['latitude'],item['longitude'],5-item[activeSortAttributeToShow+'_normalized']*5,displayText,activeSortAttributeToShow);}}
else{if(activeSortDataType=='score'){addMarkerOnMapForItem(item['long_slug'],item['short_slug'],item['name'],item['state_code'],'',item['latitude'],item['longitude'],item[activeSortAttributeToShow+'_normalized']*5,displayText,activeSortAttributeToShow);}
else if(activeSortDataType=='value'){addMarkerOnMapForItem(item['long_slug'],item['short_slug'],item['name'],item['state_code'],'',item['latitude'],item['longitude'],item[activeSortAttributeToShow+'_normalized']*5,displayText,activeSortAttributeToShow);}
else{addMarkerOnMapForItem(item['long_slug'],item['short_slug'],item['name'],item['state_code'],'',item['latitude'],item['longitude'],item[activeSortAttributeToShow+'_normalized']*5,displayText,activeSortAttributeToShow);}}}}
else{displayText=number_format(item[defaultSort],1,1);addMarkerOnMapForItem(item['long_slug'],item['short_slug'],item['name'],item['state_code'],'',item['latitude'],item['longitude'],item['nomad_score_normalized']*5,displayText,activeSortAttributeToShow);}
if(item['latitude']<-55||item['latitude']>55){}
...
if(mapFitBoundsEnabled&&!empty(data['filters'])){consoleLog('2 map.fitBounds(bounds, {');map.fitBounds(bounds,{animate:false,padding:{top:100,bottom:100,left:(100+leftPaddingBounds),right:100},duration:0});if(!isMobile){map.setZoom(map.getZoom()-1);}
...
if(count==0)count='';$('.action-clear-all-filters').text('Clear '+number_format(count)+' results');if(offsetItemsNumber){consoleLog('append items due to offsetItemsNumber='+offsetItemsNumber);$('.view.grid').append(viewGridHtmlBuffer);placeAuxItems();}
...
function downloadCSV(){consoleLog('downloadCSV');action='⌗ Downloaded CSV export';$.ajax({url:"/userApi.php",type:'POST',dataType:'json',data:{action:'log_user_activity',log_action:action,hash:md5(action+'xoyo')},context:document.body}).done(function(reply){});if(!$('html').hasClass('logged-in')){action='💳 Opened sign up modal when downloading CSV '+'('+signUpAbTestVariation+') '+'(AB variation: '+signUpAbTestVariation+')';$.ajax({url:"/userApi.php",type:'POST',dataType:'json',data:{action:'log_user_activity',log_action:action,hash:md5(action+'xoyo')},context:document.body}).done(function(reply){});openSignUpModal('📗 <b>Export data to a spreadsheet file</b> and continue your research yourself','basic');return;}
...
contents+=defaultSortText+",";contents+='Temperature (°C)'+",";contents+='Temperature (°F)'+",";contents+='Internet Speed (Mbps)'+",";contents+='Nomad Cost ($ monthly)'+",";contents+='Fun (out of 5)'+",";contents+='Safety (out of 5)'+",";}
...
if(itemData['temperatureC']==null){contents+='"'+'0°C'+'"'+",";}
else{contents+='"'+number_format(itemData['temperatureC'],0)+"°C"+'"'+",";}
if(itemData['temperatureF']==null){contents+='"'+'0°F'+'"'+",";}
else{contents+='"'+number_format(itemData['temperatureF'],0)+"°F"+'"'+",";}
...
function closeFilters(){$('html').addClass('filters-folded');$('html').animate({scrollTop:$('.top-bar').height()-20},0);setCookie('filters-folded','yes',86400*365);}
...
map.on('dragstart',function(e){$('.toolreview-ui').hide();})
...
if(empty(features)){$('.toolreview-ui').hide();map.getSource('highlight').setData({'type':'FeatureCollection','features':[]});$('.mapboxgl-canvas-container').removeClass('cursor-pointer');return;}
...
$('.toolreview-ui').text(feature['properties']['name']+': '+displayText);$('.toolreview-ui').css('top',e.point.y);$('.toolreview-ui').css('left',e.point.x-($('.toolreview-ui').width()/2));$('.toolreview-ui').show();});map.on('click',function(e){consoleLog('map.on(click, function(e){');if(!mapIs3d)return;if($('html').hasClass('city-modal-open'))return;features=map.queryRenderedFeatures(e.point,{layers:[current3dDataMapID]});if(empty(features)){return;}
...
if(empty(fittedDataExponential)){consoleLog("Finding linear fit...");requestAnimationFrame(function(){$('.loading_spinner-general .loading_message').text("📏 Finding linear fit...");$('.loading_spinner-general .loading_message').show();$('.loading_spinner-general').addClass('show');});var array=linear_regression_pairs(dataX,dataY)
...
if(!empty(pearsonCorrelationCoefficient)){statsInLegendData.push({pointRadius:0,type:'line',borderWidth:2,borderDash:[5,5],fill:false,borderColor:chartBorderColor,label:'  '+'𝑟 = '+number_format(pearsonCorrelationCoefficient,2),z:2,order:2});}
if(!empty(pValue)){statsInLegendData.push({pointRadius:0,type:'line',borderWidth:2,borderDash:[5,5],fill:false,borderColor:chartBorderColor,label:'  '+'p-value '+pValueDisplay,z:2,order:2});statsInLegendData.push({pointRadius:0,borderWidth:2,borderDash:[5,5],type:'line',fill:false,borderColor:chartBorderColor,label:'  '+'𝑛 = '+number_format(dataX.length),z:2,order:2});}}
else{}
...
console.log('dataPerRegion',dataPerRegion);var datasets=[];for(key in dataPerRegion){var labels=[];for(var i=0;i<dataPerRegion[key].length;i++){labels.push(dataPerRegion[key][i]['label']);}
